...
-S  [threads] [mmap]        Use the Julia reconstruction version (less RAM usage, different reconstruction algorithm). [threads=auto] can be auto or a number. [mmap=false] can be \"true\", \"false\" or a path. It corrects the frequency drift the protocol asks for, which the MATLAB reconstruction does not do at all; set JULIA_RECO_MATLAB_PARITY=true in the environment to drop that and the other settings MATLAB picks differently, which is what makes a run comparable with the MATLAB reconstruction of the same data.
