...
%     end
    %lukas: take this for variable TIs because we dont obtain number of points per circle now not from
    %the wipmem block
    if( isfield(ParList,'WipMemBlockInterpretation') && isfield(ParList.WipMemBlockInterpretation,'Rollercoaster')  )      % if Rollercoaster
        ParList.Dwelltimes = 2 * ParList.Dwelltimes;    % Because we have oversampling enabled, and the scanner interprets that we oversample in the spectral domain, which we dont...
    end
end
	...
	function PrescanInterpretation = CheckWipMemBlockForPrescan(ParList)
		...
		if(numel(ParList.WipMemBlock_alFree) > 55)
			try
				...
				PrescanInterpretation.NOISEADJSCAN.nReadEnc = 33; % ParList.WipMemBlock_alFree(56);   % HACK!!!
			catch
		...
		if(numel(ParList.WipMemBlock_alFree) > 58)
			PrescanInterpretation.NOISEADJSCAN.Dwelltime = 10000; % ParList.WipMemBlock_alFree(57); % HACK!!!
...
%         RollerCoasterInfoAvail = numel(ParList.WipMemBlock_alFree) > 59 && ParList.WipMemBlock_alFree(60) > 0 && ParList.WipMemBlock_alFree(1) > 0;
%         if(~RollerCoasterInfoAvail)
% 			RollercoasterInterpretation = 0;
% 			return;
% 		end
%         
        ...
        try
%             RollercoasterInterpretation.sNoADCPointsPerCircle =
%             ParList.WipMemBlock_alFree(60); %lukas this is now in the mdh
