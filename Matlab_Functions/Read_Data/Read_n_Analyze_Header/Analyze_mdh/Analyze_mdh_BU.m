...
% ParList.General.total_ADC_meas =82833; %lukas

x=0;
y=0;
z=0;
 
ParList.ONLINE.NoOfADCs = 0;
ParList.PATREFSCAN.NoOfADCs = 0;
ParList.NOISEADJSCAN.NoOfADCs = 1;
...
%         
%         
%         ParList.(EvalInfoMask_Cur).total_channel_no = chak_header(9);
%         ParList.(EvalInfoMask_Cur).Samples = chak_header(8);
%         ParList.(EvalInfoMask_Cur).SamplesAfterEcho = chak_header(8) - chak_header(38)*2;
% 
%         ParList.(EvalInfoMask_Cur).NoOfADCs = ParList.(EvalInfoMask_Cur).NoOfADCs+1;
% %         if(strcmp(Assoc,'ONLINE'))
% %             x=1;
% %         end
% %         if(strcmp(Assoc,'PATREFSCAN'))
% %             y=1;
% %           
% %         end    
% %         if(strcmp(Assoc,'NOISEADJSCAN'))
% %             z=1;
% %         end
% %         if(x && y && z)
% %             break
...
% %             fseek(fid,chak_header(8)*2*4*ParList.(Assoc).total_channel_no + (ParList.(Assoc).total_channel_no-1)*128,'cof');
%     end
...
fseek(fid,headersize,'bof');
fseek(fid,+5*4,'cof');	
EvalInfoMask_loop = fread(fid,64,'ubit1');
Assoc = Associate_EvalInfoMask(EvalInfoMask_loop);
chak_header(8:57) = fread(fid, 50, 'int16');
% fseek(fid,-50*2,'cof');
% 
% ParList.ONLINE.SamplesAfterEcho=chak_header(8) - chak_header(38)*2;
% ParList.PATREFSCAN.SamplesAfterEcho=chak_header(8) - chak_header(38)*2;
% ParList.NOISEADJSCAN.SamplesAfterEcho=chak_header(8) - chak_header(38)*2;

ParList.ONLINE.total_channel_no=chak_header(9);
ParList.PATREFSCAN.total_channel_no=chak_header(9);
ParList.NOISEADJSCAN.total_channel_no=chak_header(9);

 ParList.NOISEADJSCAN.NoOfADCs=1;
                     ParList.NOISEADJSCAN.Samples=1;
            ParList.NOISEADJSCAN.total_channel_no=1;
            ParList.NOISEADJSCAN.SamplesAfterEcho=1;
    ParList.NOISEADJSCAN.TotalRawPointsPerChannel=1;
              ParList.NOISEADJSCAN.TotalRawPoints=1;

              
 ParList.ONLINE.NoOfADCs=1;
                     ParList.ONLINE.Samples=1;
            ParList.ONLINE.total_channel_no=1;
            ParList.ONLINE.SamplesAfterEcho=1;
    ParList.ONLINE.TotalRawPointsPerChannel=1;
              ParList.ONLINE.TotalRawPoints=1;
              
 ParList.PATREFSCAN.NoOfADCs=1;
                     ParList.PATREFSCAN.Samples=1;
            ParList.PATREFSCAN.total_channel_no=1;
            ParList.PATREFSCAN.SamplesAfterEcho=1;
    ParList.PATREFSCAN.TotalRawPointsPerChannel=1;
              ParList.PATREFSCAN.TotalRawPoints=1;
