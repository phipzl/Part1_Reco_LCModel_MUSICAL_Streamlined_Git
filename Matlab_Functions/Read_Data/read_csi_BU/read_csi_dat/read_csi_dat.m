...
for CurField = transpose(fieldnames(ParList))
    ...
    % Allocate memory
    kSpace.(CurrentMeasSet) = zeros([ParList.(CurrentMeasSet).total_channel_no, ParList.(CurrentMeasSet).TotalRawPointsPerChannel],'single');
...
for CurrentMeasSet2 = transpose(fields(kSpace))
	...
	% Initialize
	for echo = minecho:maxi(6)
		Temp{echo} = zeros([Info.(CurrentMeasSet).total_channel_no maxi(2) maxi(3) maxi(4) maxi(5) samples(echo) maxi(7) maxi(8) maxi(11) maxi(12)],'single');
