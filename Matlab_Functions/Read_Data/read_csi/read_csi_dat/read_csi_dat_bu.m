function [kSpace, Info] = read_csi_dat(file, DesiredSize,ReadInDataSets,coili)
%
...
%    if(chak_header(56)==coili)  
        kSpace.(CurrentMeasSet)(1,CurPoint.(CurrentMeasSet)+1 : CurPoint.(CurrentMeasSet)+numel(chak_data)) = chak_data;
%    end
    
%     if(CurPoint.(CurrentMeasSet)+numel(chak_data) > ParList.(CurrentMeasSet).TotalRawPointsPerChannel)
%         wuff=1
%     end
...
for CurrentMeasSet2 = transpose(fields(kSpace))
	...
	% Correct for the problem that in circular encoding e.g. 63 points are
	% measured, but the matrix should be 64! BUG FÜR HAMMING WENN GENAU 255
	% kreise zbsp
% 	for dim = 2:4
...
% 	end
	...
	for echo = minecho:maxi(6)
        ...
        Temp{echo} = NaN([1 maxi(2) maxi(3) maxi(4) maxi(5) samples(echo) maxi(7) maxi(8) maxi(11) maxi(12)],'single');
	...
	for i = 1 : 1: size(Info.(CurrentMeasSet).mdhInfo,2)/Info.(CurrentMeasSet).total_channel_no 
        ...
        filli_NaN=NaN(1,size(Temp{CurEco},6)-size(filli_meas,2));
