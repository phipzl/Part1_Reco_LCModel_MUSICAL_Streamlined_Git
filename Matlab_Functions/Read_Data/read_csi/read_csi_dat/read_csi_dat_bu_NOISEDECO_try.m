function [kSpace, Info] = read_csi_dat_bu_NOISEDECO(file, DesiredSize,ReadInDataSets,coili)
%
...
if(isfieldRecursive(ParList,'WipMemBlockInterpretation','Prescan'))
	if(isfieldRecursive(ParList.WipMemBlockInterpretation,'Prescan','NOISEADJSCAN','Dwelltime') && ParList.WipMemBlockInterpretation.Prescan.NOISEADJSCAN.Dwelltime > 0)
...
% Analyze mdh.
ParList = Analyze_mdh_BU(file,1);
%Info.ONLINE.total_channel_no = ParList.total_channel_no;

if(isfield(ParList,'None'))
    ParList=rmfield(ParList, 'None');
end

if(isfield(ParList,'Other'))
    ParList=rmfield(ParList, 'Other');
end

if(isfield(ParList,'PATREFANDIMASCAN'))
    ParList=rmfield(ParList, 'PATREFANDIMASCAN');
end

if(isfield(ParList,'ACQEND'))
    ParList=rmfield(ParList, 'ACQEND');
end
...
for CurField = transpose(fieldnames(ParList))
    ...
    % Allocate memory
    kSpace.(CurrentMeasSet) = zeros([1, ParList.(CurrentMeasSet).TotalRawPointsPerChannel],'single');
    Info.(CurrentMeasSet).mdhInfo = zeros([18 ParList.(CurrentMeasSet).NoOfADCs*1]);
	...
	Info.(CurrentMeasSet).total_channel_no = 1;
    Info.NOISEADJSCAN.total_channel_no=totCoili;
...
while(~ACQEND_flag)
    ...
    if (strcmpi(CurrentMeasSet,'ONLINE') || strcmpi(CurrentMeasSet,'PATREFSCAN'))

        if coili>0
            fseek(file_fid,coili*(5*4+50*2+8+4*Info.(CurrentMeasSet).mdhInfo(9,CurInfoPt.(CurrentMeasSet))*2),'cof'); 
        end
    ...
    if(            Info.(CurrentMeasSet).mdhInfo(14,CurInfoPt.(CurrentMeasSet))==4)
        x=1
    end
    
    if (strcmpi(CurrentMeasSet,'ONLINE'))
%         if(Info.(CurrentMeasSet).mdhInfo(11,CurInfoPt.(CurrentMeasSet))==1)
%             tmp=reshape(chak_data,180,numel(chak_data)/180);
%             csi_object.(CurrentMeasSet)(Info.(CurrentMeasSet).mdhInfo(1,CurInfoPt.(CurrentMeasSet))... %coils
%                 ,Info.(CurrentMeasSet).mdhInfo(2,CurInfoPt.(CurrentMeasSet))...         %circles
%                 ,:...   %samples per temp interleave per adc
%                 ,Info.(CurrentMeasSet).mdhInfo(12,CurInfoPt.(CurrentMeasSet)):(Info.(CurrentMeasSet).mdhInfo(14,CurInfoPt.(CurrentMeasSet))-1):size(tmp,2)*(Info.(CurrentMeasSet).mdhInfo(14,CurInfoPt.(CurrentMeasSet))-1)*Info.(CurrentMeasSet).mdhInfo(11,CurInfoPt.(CurrentMeasSet))...                    %adc count
%                 ,Info.(CurrentMeasSet).mdhInfo(7,CurInfoPt.(CurrentMeasSet))...                        %averages
%                 , Info.(CurrentMeasSet).mdhInfo(6,CurInfoPt.(CurrentMeasSet)))=tmp;                         %echos
%         else
%             tmp=reshape(chak_data,180,numel(chak_data)/180);
%             csi_object.(CurrentMeasSet)(Info.(CurrentMeasSet).mdhInfo(1,CurInfoPt.(CurrentMeasSet))... %coils
%                 ,Info.(CurrentMeasSet).mdhInfo(2,CurInfoPt.(CurrentMeasSet))...         %circles
%                 ,:...   %samples per temp interleave per adc
%                 ,size(tmp,2)*(Info.(CurrentMeasSet).mdhInfo(14,CurInfoPt.(CurrentMeasSet))-1)*(Info.(CurrentMeasSet).mdhInfo(11,-1+CurInfoPt.(CurrentMeasSet)))*Info.(CurrentMeasSet).mdhInfo(12,CurInfoPt.(CurrentMeasSet)):(Info.(CurrentMeasSet).mdhInfo(14,CurInfoPt.(CurrentMeasSet))-1):size(tmp,2)*(Info.(CurrentMeasSet).mdhInfo(14,CurInfoPt.(CurrentMeasSet))-1)*Info.(CurrentMeasSet).mdhInfo(11,CurInfoPt.(CurrentMeasSet))...                    %adc count
%                 ,Info.(CurrentMeasSet).mdhInfo(7,CurInfoPt.(CurrentMeasSet))...                        %averages
%                 , Info.(CurrentMeasSet).mdhInfo(6,CurInfoPt.(CurrentMeasSet)))=tmp;    
%         end
             if(Info.(CurrentMeasSet).mdhInfo(11,CurInfoPt.(CurrentMeasSet))==1 && CurInfoPt.(CurrentMeasSet)>2)
                Info.(CurrentMeasSet).mdhInfo(2,CurInfoPt.(CurrentMeasSet)-1)
                rere=rere';
                VS=floor(size(rere,1)/360);
                rere=rere(1:VS*360);
                tmp=reshape(rere,360,size(rere,1)/360);
                
                csi_object.(CurrentMeasSet)(Info.(CurrentMeasSet).mdhInfo(1,CurInfoPt.(CurrentMeasSet)-1)... %coils
                ,Info.(CurrentMeasSet).mdhInfo(2,CurInfoPt.(CurrentMeasSet)-1)...         %circles
                ,:...   %samples per temp interleave per adc
               ,Info.(CurrentMeasSet).mdhInfo(12,CurInfoPt.(CurrentMeasSet)-1): Info.(CurrentMeasSet).mdhInfo(14,CurInfoPt.(CurrentMeasSet)-1)-1:VS*(Info.(CurrentMeasSet).mdhInfo(14,CurInfoPt.(CurrentMeasSet)-1)-1)      ...
                ,Info.(CurrentMeasSet).mdhInfo(7,CurInfoPt.(CurrentMeasSet)-1)...                        %averages
                , Info.(CurrentMeasSet).mdhInfo(6,CurInfoPt.(CurrentMeasSet)-1))=tmp;      
            ...
            rere(counter*numel(chak_data)+1:(1+counter)*numel(chak_data)) =squeeze(chak_data);
            counter = counter +1;
           
%             if(Info.(CurrentMeasSet).mdhInfo(11,CurInfoPt.(CurrentMeasSet))==1)
%                 counter = 0;
%             end
%             Info.(CurrentMeasSet).mdhInfo(14,CurInfoPt.(CurrentMeasSet))
%             numel(chak_data)
%             vsp=floor(numel(chak_data)/360);
%             tmp=reshape(chak_data,360,vsp);
...
%                 ,Info.(CurrentMeasSet).mdhInfo(2,CurInfoPt.(CurrentMeasSet))...         %circles
%                 ,:...   %samples per temp interleave per adc
%                 , counter+Info.(CurrentMeasSet).mdhInfo(12,CurInfoPt.(CurrentMeasSet)):(Info.(CurrentMeasSet).mdhInfo(14,CurInfoPt.(CurrentMeasSet))-1):counter+size(tmp,2)*(Info.(CurrentMeasSet).mdhInfo(14,CurInfoPt.(CurrentMeasSet))-1)...
%                 ,Info.(CurrentMeasSet).mdhInfo(7,CurInfoPt.(CurrentMeasSet))...                        %averages
%                 , Info.(CurrentMeasSet).mdhInfo(6,CurInfoPt.(CurrentMeasSet)))=tmp;                         %echos
%         
%                counter = counter +size(tmp,2)*(Info.(CurrentMeasSet).mdhInfo(14,CurInfoPt.(CurrentMeasSet))-1);

    end
    
    
    if (strcmpi(CurrentMeasSet,'ONLINE') || strcmpi(CurrentMeasSet,'PATREFSCAN'))
        kSpace.(CurrentMeasSet)(1,CurPoint.(CurrentMeasSet)+1 : CurPoint.(CurrentMeasSet)+numel(chak_data)) = chak_data;
    else
    ...
    if(strcmpi(CurrentMeasSet,'NOISEADJSCAN') )
    ...
    if (strcmpi(CurrentMeasSet,'ONLINE') || strcmpi(CurrentMeasSet,'PATREFSCAN'))
        fseek(file_fid,(totCoili-coili-1)*(5*4+50*2+8+4*Info.(CurrentMeasSet).mdhInfo(9,CurInfoPt.(CurrentMeasSet))*2),'cof');
    end
...
Info.General.Ascconv.WipMemBlockInterpretation.Rollercoaster.sNoADCPointsPerCircle = ( Info.ONLINE.mdhInfo(13,1) - 1);
% find out ... how many adcs per TI
index_of_adc=find(Info.ONLINE.mdhInfo(11,:)==1)-1;
max_adc_per_TI=Info.ONLINE.mdhInfo(11,index_of_adc(2:end));
max_adc_per_TI=fliplr(unique(max_adc_per_TI));

% ... how many adc with per TI
for i=1:max(Info.ONLINE.mdhInfo(14,:))-1
    bong(i)=numel(Info.ONLINE.mdhInfo(14,(find(Info.ONLINE.mdhInfo(14,:)==i+1))))/i;
end

    
%find out he number of temporal interleaves for each circle! (also 3D)          
[C ia ib]=unique(Info.ONLINE.mdhInfo(2,:),'last');
NoOfTempInterleaves=Info.ONLINE.mdhInfo(11,ia);

for i=1:max(Info.ONLINE.mdhInfo(14,:))-1
    NoOfTempInterleaves(NoOfTempInterleaves==max_adc_per_TI(i))=i;
end
...
% 	for dim = 2:4 %%%%% here is a bUGGGGGG: vor var TI and hamming, it adds one circle!!!!
...
% 	end
	...
	% Initialize
	for echo = minecho:maxi(6)
		%Temp{echo} = zeros([1 maxi(2) maxi(3) maxi(4) maxi(5) samples(echo) maxi(7) maxi(8) maxi(11) maxi(12)],'single');
        Temp{echo} = NaN([Info.(CurrentMeasSet).total_channel_no maxi(2) maxi(3) maxi(4) maxi(5) samples(echo) maxi(7) maxi(8) maxi(11) maxi(12)],'single');
	...
	for i = 1 : 1: size(Info.(CurrentMeasSet).mdhInfo,2)/Info.(CurrentMeasSet).total_channel_no 
        ...
        filli_meas=kSpace.(CurrentMeasSet)(:,CurPoint.(CurrentMeasSet)+1:CurPoint.(CurrentMeasSet)+Info.(CurrentMeasSet).mdhInfo(9,i)-Info.(CurrentMeasSet).mdhInfo(10,i));
        filli_NaN=NaN(Info.(CurrentMeasSet).total_channel_no ,size(Temp{CurEco},6)-size(filli_meas,2));
        Temp{CurEco}(:,Curkx, Curky, Curkz, CurSlc, 1:size(filli_meas,2), CurAvg, CurRep, CurADC,CurTempIntNo) = filli_meas;
        Temp{CurEco}(:,Curkx, Curky, Curkz, CurSlc, size(filli_meas,2)+1:size(Temp{CurEco},6) , CurAvg, CurRep, CurADC,CurTempIntNo) = filli_NaN;
   
          end
          
            if(strcmpi(CurrentMeasSet,'NOISEADJSCAN') )
%for variable TIs we take the thing above; lukas
        Temp{CurEco}(:,i,:) = ...
	...
	kSpace = rmfield(kSpace,CurrentMeasSet); 
	...
	for echo = minecho:maxi(6)
		...
		if(isfieldRecursive(Info,'General','Ascconv','WipMemBlockInterpretation','Rollercoaster','sNoADCPointsPerCircle') && ~strcmpi(CurrentMeasSet,'NoiseAdjScan'))
            ...
            %set adcs which were not measured to NaN 
           
            
           
            points_per_variablke_tempInt=sort(unique(sum((~isnan(Temp{echo}(1,:,1,1,1,1,:,1))),7)),'descend');
            for i=1:size(Temp{echo},6)
                vecSize(i)=(points_per_variablke_tempInt(i)*i/(Info.General.Ascconv.WipMemBlockInterpretation.Rollercoaster.sNoADCPointsPerCircle*2));    
                vecSize(i) = floor(vecSize(i)/i);
                vecSize(i) = vecSize(i)*i;     
			...
			vecSize_bu=vecSize;
            for i= 1: numel(vecSize_bu)
            
                vecSize(find(NoOfTempInterleaves==i))=vecSize_bu(i);
            end
            clear vecSize_bu
            %vecSize=repmat(vecSize(1), [1 length(NoOfTempInterleaves)]);
           
            for i=1:size(Temp{echo},2) % for variable TI and Hamming it seems that we allocated 1 circle to much
                
                % Same here
                TakeOnlyPoints = ceil(vecSize(i)./NoOfTempInterleaves(i))*NoOfPtsPerLoop;		% vecSize/NoOfTempInterleaves ... Spectral Points, NoOfPtsPerLoop ... PointsPerLoop
                circ{i} = Temp{echo}(:,i,:,:,:,1:NoOfTempInterleaves(i),:,:,:);
                
                bla=squeeze(circ{i}(1,:,:,:,:,1,:,1,:)); % the position of Nans doesnt change inbetween  channels, relevent TIs, averages
                pos=isnan(bla);
    
                circ{i}(:,:,:,:,:,:,pos,:,:)=[];
                clear pos bla
                circ{i}=circ{i}(:,:,:,:,:,:,1:TakeOnlyPoints,:,:);
                
                circ{i} = reshape(circ{i},[size_MultiDims(circ{i},1:5) NoOfTempInterleaves(i) NoOfPtsPerLoop ceil(vecSize(i)/NoOfTempInterleaves(i)) size_MultiDims(circ{i},8:9)]);
                circ{i} = permute(circ{i},[1 2 3 4 5 7 6 8 9 10]);
                circ{i} = reshape(circ{i},[size_MultiDims(circ{i},1:5) NoOfPtsPerLoop NoOfTempInterleaves(i)*ceil(vecSize(i)/NoOfTempInterleaves(i)) size_MultiDims(circ{i},9:10)]);
                % Permute dimensions to make size {echo}(cha x LoopNo x PointsPerLoops x kz x slice x samples x avg x rep)
                circ{i} = permute(circ{i},[1 2 6 4 5 7 8 9 3]);
            
                circ{echo}(:,i,:,:,:,1:size(circ{i},6),:)=circ{i};
            end
...
%             
%             vecSize=floor(size(Temp{echo},7)*size(Temp{echo},6)/(Info.General.Ascconv.WipMemBlockInterpretation.Rollercoaster.sNoADCPointsPerCircle*2));
%           
%             if( isfieldRecursive(Info,'General','Ascconv','WipMemBlockInterpretation','Rollercoaster','sNoADCPointsPerCircle') && Info.General.Ascconv.WipMemBlockInterpretation.Rollercoaster.sNoADCPointsPerCircle > 0 )
% 				NoOfPtsPerLoop = Info.General.Ascconv.WipMemBlockInterpretation.Rollercoaster.sNoADCPointsPerCircle*2*1;				% *last entry ... Spatial oversampling. always *2 internal
% 			else	% otherwise make a guess 
...
% 			end
%             
%             NoOfTempInterleaves = maxi(12);	
%             
%             %round correctly
%             vecSize = floor(vecSize/NoOfTempInterleaves);
%             vecSize = vecSize*NoOfTempInterleaves;
%             
%             %round ending
%             
%             
...
%             
	...
	kSpace.(CurrentMeasSet) = Temp;	clear Temp;
