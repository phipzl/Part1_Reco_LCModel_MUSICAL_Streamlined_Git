function w_dif = f_shift_H2O_1_2(d,threshold,dt)
...
for k = 1:N_fid
    ...
    if(min(temp(1,i_start_H2O:i_fin_H2O)) == max(temp(1,i_start_H2O:i_fin_H2O)))        % If the FID & spectrum only contains zeros (for whatever reason...)
        w_dif_temp = 0;
    else
        w_dif_temp      = w_H2O_ref - w(temp == H2O);
    end
    clear H2O
    
    
    
    if(abs((mod(w_dif_temp,dw_store)/dw_store)) > 0.5)
        w_dif(k) = round(w_dif_temp/dw_store)*dw_store;
    else
        w_dif(k) = fix(w_dif_temp/dw_store)*dw_store;
    end
    clear temp
