...
MB = data.current_header_pars.MB;
data.true_multi_echo='yes'; %(set to 'no' for CURED-EPI, where the second echo should be treated as a different time point)
...
extra_info_string = '';
if strcmp(nucleus,'1H') ~=1
    extra_info_string = [extra_info_string nucleus ', '];
end

if isvar('dcminfo.ImageType') ~=0
...
switch sequence_type
    ...
    case {'lnif_epi_exp','lnif_epi_devel','ep2d_bold','ep2d_pace','ep2d_fid','ep2d_pasl','ep2d_se','mz_ep2d_bold_dc','lnif_epi_bold_dc_1','mz_ep2d_bold_dc_DiCo','BP_ep_multipurpose','ep2d_bold_sine_676b'}
        dim_phase=int16(dim_read*(dcminfo.PercentPhaseFieldOfView/100));
    ...
    case {'bow_ep2d_varTE_4','bow_ep2d_varTE_5','bow_ep2d_varTE_6'}
        dim_phase=int16(dim_read*(dcminfo.PercentPhaseFieldOfView/100));
        fprintf(data.fp_scanlog, 'Scan %s: CURED-EPI %s. %ix%ix%i,%s(%.0f),TE={%sms},TR=%.0f,FA=%.0f,RWB=%i,NR=%i,%sTA=%ss,NF=%i\n', current_scan, ProtocolName, dim_read, dim_phase, dim_nslices, orientation, plane_angle, TE_string, TR, FA, RBW, NR, extra_info_string, TA, NF);
        data.true_multi_echo='no'; %(because in CURED EPI the second echo should be treated as a different time point)
    case {'BP_WIP_ep2d_bold_multiecho','BP_ghostfree_EPI','BP_IDEA_EPI_zshim'}        %ME-EPI: 'BP_WIP_ep2d_bold_multiecho' is what is used for DOCMA and it follows the same rules
        dim_phase=int16(dim_read*(dcminfo.PercentPhaseFieldOfView/100));
        fprintf(data.fp_scanlog, 'Scan %s: MEPI %s. %ix%ix%i,%s(%.0f),NE=%i{%sms},TR=%.0f,FA=%.0f,RWB=%i,NR=%i,%sTA=%ss,NF=%i\n', current_scan, ProtocolName, dim_read, dim_phase, dim_nslices, orientation, plane_angle, ne, TE_string, TR, FA, RBW, NR, extra_info_string, TA, NF);
        data.true_multi_echo='no'; %does this need another flag, depending on how the echoes are combined?
    case {'ep2d_pace_492A'}      
        dim_phase=int16(dim_read*(dcminfo.PercentPhaseFieldOfView/100));
        fprintf(data.fp_scanlog, 'Scan %s: PLACE EPI %s. %ix%ix%i,%s(%.0f),NE=%i{%sms},TR=%.0f,FA=%.0f,RWB=%i,NR=%i,%sTA=%ss,NF=%i\n', current_scan, ProtocolName, dim_read, dim_phase, dim_nslices, orientation, plane_angle, ne, TE_string, TR, FA, RBW, NR, extra_info_string, TA, NF);
        data.true_multi_echo='no'; %does this need another flag, depending on how the echoes are combined?
    case {'ep2d_bold_SliceAcc_770A'}
        dim_phase=int16(dim_read*(dcminfo.PercentPhaseFieldOfView/100));
        fprintf(data.fp_scanlog, 'Scan %s: MB EPI %s. %ix%ix%i,%s(%.0f),MB=%i,TE/TR=%.0f/%.0f,FA=%.0f,RWB=%i,NR=%i,%sTA=%ss,NF=%i\n', current_scan, ProtocolName, dim_read, dim_phase, dim_nslices, orientation, plane_angle, MB, TE, TR, FA, RBW, NR, extra_info_string, TA, NF);
    ...
    case {'gre','se_mc','ob_gre','CV_shim_450B','gre_phs_filt_669','gre_mgh_multiecho','bow_ph_map_3'}
    ...
    case {'is_doublegre_130506b'}
        fprintf(data.fp_scanlog, 'Scan %s: Double GRE %s. %ix%ix%i,NF=%i, %s \n',current_scan,ProtocolName, dim_read, dim_phase, dim_nslices, NF, extra_info_string);
    case {'os_mepi2d'}
    ...
    case {'tse','tse_vfl_WIP692'}
        fprintf(data.fp_scanlog, 'Scan %s: TSE %s. %ix%ix%i%s,TE/TI/TR=%i/%i/%i, NF=%i\n', current_scan, ProtocolName, dim_read, dim_phase, dim_nslices, extra_info_string, TE, TI, TR, NF);
    case {'tfl'}
        fprintf(data.fp_scanlog, 'Scan %s: MPRAGE(TFL) %s. %ix%ix%i%s, NF=%i\n', current_scan, ProtocolName, dim_read, dim_phase, dim_nslices, extra_info_string, NF);
    case {'se'}
    ...
    case {'tse_vfl','BS_LS_tse020712'}
        fprintf(data.fp_scanlog, 'Scan %s: TSE %s. %ix%ix%i%s, NF=%i\n', current_scan, ProtocolName, dim_read, dim_phase, dim_nslices, extra_info_string, NF);
    case {'CV'}
        fprintf(data.fp_scanlog, 'Scan %s: CV %s. %ix%ix%i%s\n', current_scan, ProtocolName, dim_read, dim_phase, dim_nslices, extra_info_string);
    otherwise
        if strcmp(data.raw_data, 'no') == 1
            disp(['Never tried to convert ' sequence_type ' before.  Writing some basic parameters. Consider configuring write_scan_details.m. ']);
            fprintf(data.fp_scanlog, 'Scan %s: ??? %s. %ix%ix%i, %s\n', current_scan, ProtocolName, dim_read, dim_phase, dim_nslices, extra_info_string);
        end
