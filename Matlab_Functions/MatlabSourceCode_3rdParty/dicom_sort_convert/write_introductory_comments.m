...
if isvar('dcminfo.DerivationDescription')
    ...
    % patient age - dcminfo.PatientAge is just to the nearest year
    dob_full = dcminfo.PatientBirthDate;
    acqdate_full =  dcminfo.AcquisitionDate;
        % reformat dob and acqdate into date structures
    dob(1) = str2num(dob_full(1:4));
    dob(2) = str2num(dob_full(5:6));
    dob(3) = str2num(dob_full(7:8));
    dob(4:6) = 0;
    acqdate(1) = str2num(acqdate_full(1:4));
    acqdate(2) = str2num(acqdate_full(5:6));
    acqdate(3) = str2num(acqdate_full(7:8));
    acqdate(4:6) = 0;
    age = etime(acqdate, dob)/(60*60*24*365.25);
    fprintf(data.fp_scanlog, 'Age: %.2f years\n', age);
    fprintf(data.fp_scanlog, 'Gender: %s \n', dcminfo.PatientSex);
end
try
