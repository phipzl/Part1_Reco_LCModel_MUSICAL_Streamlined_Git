...
if(nargin < 2)
	fprintf('\nError in CalcSynthMap.m: Too little input. Aborting.\n')
	return
end
...
if(numel(MetMapTitle) < numel(SynthMapTitle))
	fprintf('\nError in CalcSynthMap.m: Did not find all necessary metabolic maps. Aborting.\n')
	return
end

MetaboInd = regexp_SearchCellInCell(MetMapTitle,SynthMapTitle);

% Check if metabos occur several times
if( sum(cellfun(@numel,MetaboInd) > 1) ~= 0 )
	fprintf('\nUnexpected Error in CalcSynthMap.m: At least one entry of SynthMapTitle occurs several times in MetMapTitle.\nAbort.')
	return
end

% Check if all metabos exist
if(sum(~cellfun(@isempty,MetaboInd)) < numel(SynthMapTitle))
	fprintf('\nError in CalcSynthMap.m: Did not find all necessary metabolic maps. Aborting.\n')
	return
end
