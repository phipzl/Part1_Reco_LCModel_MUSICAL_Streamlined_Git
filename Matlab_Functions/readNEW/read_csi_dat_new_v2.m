function [kSpace, Info] = read_csi_dat_new_v2(file, tmp_dir)
% Adapted by PL 
% ... now twice! (Nov 2021)
    ...
    fprintf('Saving MeasurementInfos.txt to tmp...');
    
    PatientData=[inData.hdr.Dicom.tPatientName, ' ', ...
    num2str(inData.hdr.Config.PatientBirthDay), ' ', ...
    num2str(inData.hdr.Dicom.flPatientAge), ' ', ...
    num2str(inData.hdr.Dicom.lPatientSex), ' ', ...
    num2str(inData.hdr.Dicom.flUsedPatientWeight), ' ', ...
    num2str(inData.hdr.Dicom.lFrequency), ' ', ...
    num2str(inData.hdr.Dicom.flMagneticFieldStrength), ' ', ...
    num2str(inData.hdr.Dicom.flTransRefAmpl), ' ', ...
    num2str(inData.hdr.Dicom.adFlipAngleDegree)];
    
    fid=fopen([tmp_dir '/MeasurementInfos.txt'], 'w');
