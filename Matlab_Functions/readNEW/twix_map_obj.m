...
%         end
    ...
    function this = clean(this)
        ...
        this.NPhs = 1;%max(this.Phs);
        this.NEco = max(this.Eco);
        this.NRep = max(this.Rep);
        this.NSet = max(this.Set);
        this.NSeg = 1;%max(this.Seg);
        this.NIda = max(this.Ida);
        this.NIdb = max(this.Idb);
        this.NIdc = 1;%max(this.Idc);
        this.NIdd = 1;%max(this.Idd);
        this.NIde = 1;%max(this.Ide);
        this.NPointsPerCircle = max(this.PointsPerCircle)-1;this.PointsPerCircle=[];
    ...
    function varargout = subsref(this, S)
        ...
        if this.arg.doAverage
            ...
            cIx( 4,:) = 1;%this.Ave(cIxToRaw);
        end
        cIx( 5,:) = 1;%this.Phs(cIxToRaw);
        ...
        if this.arg.ignoreSeg
            ...
            cIx( 9,:) = 1;%this.Seg(cIxToRaw);
        end
        cIx(10,:) = this.Ida(cIxToRaw);
        cIx(11,:) = this.Idb(cIxToRaw);
        cIx(12,:) = 1;%this.Idc(cIxToRaw);
        cIx(13,:) = 1;%this.Idd(cIxToRaw);
        cIx(14,:) = 1;%this.Ide(cIxToRaw);
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	fprintf(' Reading data using a fixed block size... ');
        ...
        pause(1)
        fprintf('\nCreating complex array "out"... ');
        out = complex(zeros(outSize,'single'));                     % memory load
        % Dimensions: FID x Number of Channels x Block Size (?)
        fprintf('Done.\n');
        pause(1)
        ...
        % machine an data.
        blockSz   = 2048; %2;           % size of blocks; must be 2^n; will be increased
        doLockblockSz = true;   % false % whether blockSZ should be left untouched
        ...
        fprintf('Going through the memory and reading data...\nExpensive computations follow...\n');
        for k = 1:kMax %    % Significant memory load here... Perhaps save by parts to disk
            % skip scan header
            ...
            % Note: on the first iteration, raw has a size of 2 MB 
            ...
            if blockCtr == blockSz || k == kMax || (isBrokenRead && blockCtr > 1)
                ...
                fprintf('%6i ', k); 	% Expensive computations...  
                ...
                count_ave(idx1) = count_ave(idx1) + 1;  % set count_ave for non-duplicates to one
                
                % Write to output variable 'out'
                % Depending on whether the block is a duplicate or not (?)
                if isempty( idxN )          % there are no duplicates
                    % no duplicates
                    if all( count_ave(idx1) == 1 )  % first acquisition of this line    % are all values of count_ave == 1? <-> there are only non-duplicates
                        out(:,:,idx1) = block;                              % fast      % replace block
                    else                                                                % there are some duplicates
                        out(:,:,idx1) = out(:,:,idx1) + block;              % slow      % add block
                    end
                else                        % there are duplicates
                    out(:,:,idx1) = out(:,:,idx1) + block(:,:,~isDupe);     % slower    % add blocks that are not duplicates

                    block = block(:,:,isDupe);                                          % block now contains just duplicates
                    for n = 1:numel(idxN)   % number of duplicates
                        out(:,:,idxN(n)) = out(:,:,idxN(n)) + block(:,:,n); % snail :-) % add duplicates
                        count_ave(idxN(n)) = count_ave(idxN(n)) + 1;                    % set count_ave = 2 for duplicates that have been added
                    end
                end
                 
%                 % Check memory usage after some iterations
%                 if k > round(kMax/2) && breakonce == 0
%                     %whos
%                     pause(1)
%                     breakonce = 1;
%                 end
...
%                 fileCtr = fileCtr + 1;
%                 fprintf('Creating h5 file out_%04i.h5...\nSize of out is ', fileCtr);
%                 fprintf('%i ', size(out));
%                 fprintf('\n');
%                 tmpfolder = '/ceph/mri.meduniwien.ac.at/departments/radiology/mrsbrain/lab/Sourcecode/MRSI_Processing_ReleaseVersions/plazen/tmp';
%                 outfilename = sprintf('%s/out_%04i.h5', tmpfolder, fileCtr);
%                 outdatasetname = sprintf('/data'); %_%0i', fileCtr);
%                 fprintf('Create h5. ');
%                 h5create(outfilename, outdatasetname, size(out));
%                 fprintf('Write h5. ');
%                 h5write(outfilename, outdatasetname, out);
%                 fprintf('Done. \n\n');
%                 clear out outfilename outdatasetname 
