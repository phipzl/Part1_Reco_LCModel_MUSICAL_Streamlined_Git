...
%         3) Age listed more accurately in scan_list.txt 
%         4) Gender included in scan_list.txt 
%   v1.19 1) Fixed a bug in the number of coil elements identified
%         2) Now writes out some basic parameters for unknown scan types
%   v1.20 1) Crash for T1 maps elliminated
%   v1.21 1) Puts unidentified integer factors (e.g. nsets) into a futher scan dimension in the reformatting function
%         2) Writes the nucleus for X-nucleus scans
%         3) Added multiband details (Barbara Dymerska)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%   these choices are overwritten by data.* parameters in function calls to dicom_sort_convert_main
    ...
    function data = set_data_defaults
        data.anonymise = 'no';
        data.test = 'yes'; % no = move data to ima_to_delete directory after sort/convert, delete scan directories in list mode
        data.define_writefile_subdir = 'yes';
        data.version_number = '1.21';
        ...
        switch data.user
            case 'Simon'
                data.default_startpath = '/dfh/raid/data/simon/acquired_data/sorting_spot';
            otherwise
        end
