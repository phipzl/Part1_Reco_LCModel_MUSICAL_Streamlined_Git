...
switch data.test
    case 'yes'        
        disp(sprintf('Dicom sort-convert is running in test mode. If sorting was successful, tar and zip IMA with the command\n$ tar cvfz %s %s/*IMA --remove-files',  fullfile(data.writefile_dir_selected, data.anon_subdir, 'ima.tar.gz'), data.readfile_dir));
    case 'no'
        tic
