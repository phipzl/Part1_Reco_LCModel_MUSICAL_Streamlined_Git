function [OutArray,ChebyFilter] = ChebyFilter(OutArray,ApplyAlongDims,FilterWidth,RadialOrOuterProduct,InputIskSpace_flag)
%
...
% -         ChebyFilter              ...    The values of the Hamming filter in k-Space.
%
%
...
if(strcmpi(RadialOrOuterProduct,'Radial'))
	...
	ChebyFilter = NaN(Size_OutArray(ApplyAlongDims));
	...
	for LinLoopy = LinIndex
		...
		[MatInd{:}] = ind2sub(size(ChebyFilter),LinLoopy);
		MatInd2 = [MatInd{:}] - (size(ChebyFilter)+1)/2;
		
		% Dont process anything that is outside of the ellipsoid (consider ellipse equation!)
		if(sum((MatInd2 ./ (size(ChebyFilter)/2)).^2) > 1 )
			continue;
		end
		
		if(	sum((MatInd2 ./ OnesRadius).^2) <= 1 )
			ChebyFilter(LinLoopy) = 1;
		else
			ChebyFilter(LinLoopy) = sum((MatInd2./FilterSize).^2);
		end
		
	end

	ChebyFilter(ChebyFilter ~= 1) = 0.54 + 0.46*cos(2*pi*(sqrt(ChebyFilter(ChebyFilter ~= 1)) - NormFactor));		
    %ChebyFilter(ChebyFilter ~= 1) = 0.35875 - 0.48829*cos(2*pi*(sqrt(ChebyFilter(ChebyFilter ~= 1)) - NormFactor))+0.14128*cos(4*pi*(sqrt(ChebyFilter(ChebyFilter ~= 1)) - NormFactor))-0.01168*cos(6*pi*(sqrt(ChebyFilter(ChebyFilter ~= 1)) - NormFactor));	
	
	% Extrapolate the filter to the edges
	HammingSize = size(ChebyFilter);
	for hamming_dim = 1:numel(ApplyAlongDims)
		ZerosSize = size(ChebyFilter); ZerosSize(hamming_dim) = 1;
		ChebyFilter = cat(hamming_dim,zeros(ZerosSize),ChebyFilter,zeros(ZerosSize));
	end
	ChebyFilter = inpaint_nans(ChebyFilter,4);
	ChebyFilter = reshape(ChebyFilter,HammingSize+2); %#ok
	% Crop out the good data
	CropString = '2:end-1,'; CropString = repmat(CropString,[1 numel(ApplyAlongDims)]); CropString(end) = [];
	ChebyFilter = eval(['ChebyFilter(' CropString ');']);
	
	
	% Replicate To size of OutArray
	ChebyFilter = myrepmat(ChebyFilter,Size_OutArray);
	
	
	
else
	
	ChebyFilter = ones(size(OutArray));	
	for hamming_dim = ApplyAlongDims                                            % Compute Hamming filter in each dimension seperately
		...
		ChebyFilter = repmat(hamming_1D_LeadingOnes, RepmatToSizeOfMatrixIn) ...  % Replicate 1d-Hamming to the matrix size, 
						.* ChebyFilter;                                           % Multiply this array with the previous calculated array
