...
%     end
    ...
    % Sum averages
    if(isfield(PreProcessingInfo.(CurDataSetString),'SumAverages_flag') && PreProcessingInfo.(CurDataSetString).SumAverages_flag && size(kSpace.(CurDataSetString){1},7) > 1) 
        AvgTemp = zeros(size(kSpace.(CurDataSetString){1}(:,:,:,:,:,:,1)));
        for avg = 1:size(kSpace.(CurDataSetString){1},7)
            AvgTemp = AvgTemp + kSpace.(CurDataSetString){1}(:,:,:,:,:,:,avg);
        end
        kSpace.(CurDataSetString){1} = AvgTemp / size(kSpace.(CurDataSetString){1},7); clear AvgTemp;
    end
