...
ControlWrite.PPMREF = {'PPMREF(1,2) = 3.03'};                        % PPMREF(JREF,JCCF): What chemical shift should the delta function with number JREF for the Reference with number JCCF have?
%ControlWrite.PPMREF = {'PPMREF(1,2) = 1.52'};                        % PPMREF(JREF,JCCF): What chemical shift should the delta function with number JREF for the Reference with number JCCF have?

ControlWrite.SHIFMNMX = {'SHIFMN(2) = -0.3','SHIFMX(2) = 0.3'};  % SHIFMN(JCCF): Minimum chemshift to search for peak. LCModel searches from PPMREF(2,1) + SHIFMN(2) to PPMREF(2,1) + SHIFMX(2)
                                                                  % SHIFMN should be negative.             

%ControlWrite.PPMSHF = 'PPMSHF = -8';                                                      
